...
#define MSG_FLAG_NO_REFLECT         (1 << (MSG_LOCAL_FLAG_SHIFT + 1))
...
public:
    ...
    void setNoReflect(bool active)
    {
        if (active)
        {
            mFlag |= MSG_FLAG_NO_REFLECT;
        }
        else
        {
            mFlag &= ~MSG_FLAG_NO_REFLECT;
    ...
    bool isNoReflect() const
    {
        return !!(mFlag & MSG_FLAG_NO_REFLECT);
    }
