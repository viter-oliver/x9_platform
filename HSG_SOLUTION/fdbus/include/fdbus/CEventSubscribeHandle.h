...
// bit fields for 'flag'
#define FDB_SUBFLAG_NO_REFLECT      (1 << 0)
...
public:
    ...
    void subscribe(CFdbSession *session, FdbMsgCode_t msg, FdbObjectId_t obj_id,
                    const char *topic, uint32_t flag);
...
private:
    ...
    bool broadcastOneMsg(CFdbSession *session, CFdbMessage *msg, CSubscribeItem &sub_item,
                         FdbSessionId_t sender_sid, FdbObjectId_t sender_oid);
};
